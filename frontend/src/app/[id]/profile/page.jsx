...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/user/${userId}`);
        if (response.ok) {
    ...
    const fetchUserSajaks = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/sajak/user/${userId}`);
        if (response.ok) {
                  ...
                  <article key={sajak._id} className="flex flex-col">
                    ...
                    <div className="flex items-start justify-between gap-3 md:gap-6 mb-4 md:mb-6">
                      ...
                      <div className="flex gap-3 flex-shrink-0 pt-2">
                        <Link href={`/${userId}/sajak/edit/${sajak._id}`}>
                          <button 
